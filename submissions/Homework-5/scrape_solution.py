...
def get_avg_rating(hotel_ratings):
    i = 5
    totalRatings = 0
    linearModel = 0
    for rating in hotel_ratings:
        ...
        rating = int(rating)
        totalRatings += rating
        linearModel += (i * rating)
        i -= 1
    return linearModel / totalRatings
...
def parse_hotel_info(hotel_url):
    ...
    rating_col = {'overall_ratings': [], 'categorical_ratings': [], 'avg': 0}
    for box in rating_summary:
        ...
        if box.get('class') == 'barChart':
            ...
            for bar in barChart:
                ...
                # gather number of reviews for each category
                hotel_ratings.append(count)
                # log.info(label + ": " + count)
            hotel_ratings =  [x.encode('UTF8') for x in hotel_ratings]
            avg_rating = get_avg_rating(hotel_ratings)
            rating_col['overall_ratings'] = hotel_ratings
            rating_col['avg'] = avg_rating

        # Rating Summary
        else:
            # log.info(' ##### Rating Summaries ##### ')
            rating_fields = box.findAll('li')
            hotel_category_ratings = []
            for row in rating_fields:
                label = row.find('div', {'class': 'name'}).find(text=True)
                count = row.find('img')['alt']
                hotel_category_ratings.append(count)
                # log.info(label + ": " + count)
            rating_col['categorical_ratings'] = hotel_category_ratings

    total_hotel_ratings.append(rating_col)
